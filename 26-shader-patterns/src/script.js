...
const geometery = new THREE.PlaneGeometry(1, 1, 32, 32)
const count = geometery.attributes.position.count
const randoms = new Float32Array(count)
//给属性添加新值
for (let i = 0; i < count; i++) {
    randoms[i] = Math.random()
}
geometery.setAttribute('aRandom', new THREE.BufferAttribute(randoms, 1))
...
gui.add(material.uniforms.uFrequency.value, 'x', 0, 20, 0.001).name('uFrequencyX')
gui.add(material.uniforms.uFrequency.value, 'y', 0, 20, 0.001).name('uFrequencyY')
